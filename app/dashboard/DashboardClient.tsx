...
// KPI Cards + Kanban Board in einem Client Component → live update bei Drag & Drop
...
function getHealth(r: DashboardRepair): { text: string; cls: string } | null {
  ...
  if (r.boardStatus === "Rückfrage Kunde"     && ud != null && ud >= 3) return { text: `${ud}d ohne Reaktion`, cls: "text-red-700 bg-red-50 border-red-200" };
  if (r.boardStatus === "Ersatzteil bestellt" && ud != null && ud >= 4) return { text: `${ud}d unverändert`,   cls: "text-amber-700 bg-amber-50 border-amber-200" };
...
function KpiCards({ items }: { items: DashboardRepair[] }) {
  const openCount    = items.filter(r => ["Angenommen","In Diagnose","Rückfrage Kunde","Ersatzteil bestellt","In Reparatur"].includes(r.boardStatus)).length;
  const rueckfrage   = items.filter(r => r.boardStatus === "Rückfrage Kunde").length;
  ...
  const inProgress   = items.filter(r => ["In Diagnose","Ersatzteil bestellt","In Reparatur","Rückfrage Kunde"].includes(r.boardStatus)).length;

  return (
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
      <KpiCard label="Aktive Aufträge"  value={openCount}  dotColor="bg-amber-400"   hint={`${inProgress} in Bearbeitung`} />
      <KpiCard label="Rückfrage Kunde"  value={rueckfrage} dotColor="bg-red-500"     urgent />
      <KpiCard label="Abholbereit"      value={readyCount} dotColor="bg-emerald-500" hint="Warten auf Abholung" />
      <KpiCard label="Abgeschlossen"    value={doneCount}  dotColor="bg-gray-400"    hint={`von ${items.length} gesamt`} />
    </div>
  );
}
    ...
    <section ref={setNodeRef}
      ...
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-inherit">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] font-semibold text-gray-700 truncate max-w-[120px]">{status}</span>
        </div>
...
export default function DashboardClient({ initialItems }: { initialItems: DashboardRepair[] }) {
  ...
  async function updateStatus(repairId: string, nextStatus: DashboardStatus) {
    ...
    const prev = [...items];
    setError(null);
    // ── Sofort im UI updaten (optimistic) ──
    setItems((p) => p.map((i) =>
    ...
    try {
      const res  = await fetch(`/api/repairs/${repairId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: STATUS_VALUE_MAP[nextStatus] }) });
  ...
  return (
    <div className="space-y-4">
      {/* ── KPI Cards – live aus items State ── */}
