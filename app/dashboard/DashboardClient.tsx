...
export type DashboardRepair = {
  ...
  kunden_name: string | null; hersteller: string | null; modell: string | null;
  geraetetyp: string | null; reparatur_problem: string | null;
  annahme_datum: string | null; updated_at: string | null;
...
function getHealth(r: DashboardRepair) {
...
function KpiCard({ label, value, dotColor, hint, urgent }: {
  ...
  return (
    <div className={`rounded-2xl p-5 border transition-all ${urgent && value > 0 ? "bg-red-50 border-red-200" : "bg-white border-gray-200 shadow-sm"}`}>
      <div className="flex items-center gap-2 mb-3">
        <span className={`w-2 h-2 rounded-full ${dotColor} ${urgent && value > 0 ? "animate-pulse" : ""}`} />
        <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">{label}</span>
      </div>
      <div className={`text-4xl font-semibold tracking-tight ${urgent && value > 0 ? "text-red-600" : "text-gray-900"}`}>{value}</div>
...
function RepairCard({ repair, saving, overlay = false, onStatusChange, isNew }: {
  ...
  return (
    <div ref={overlay ? undefined : setNodeRef}
      ...
      <div ref={active ? card3dRef : undefined}
        ...
        <div className={`bg-white rounded-xl border overflow-hidden transition-all ${
          isNew ? "border-green-400 ring-2 ring-green-200" : "border-gray-200"
        } ${overlay ? "shadow-2xl ring-1 ring-gray-300 scale-[1.02]" : "shadow-sm"}`}>
          <div className="h-[3px]" style={{ backgroundColor: STATUS_STRIPE[repair.boardStatus] }} />
          <div className="p-3 space-y-2">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <div className="text-sm font-semibold text-gray-900 truncate">{repair.hersteller || "Gerät"} {repair.modell || ""}</div>
                  {isNew && <span className="text-[9px] font-bold bg-green-500 text-white px-1.5 py-0.5 rounded-full shrink-0">NEU</span>}
                </div>
            ...
            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[11px] font-semibold ${cfg.pill}`}>
              <span className={`w-1.5 h-1.5 rounded-full ${cfg.dot}`} />{repair.boardStatus}
            </span>
            {health && <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg border text-[11px] font-medium ${health.cls}`}>⚠ {health.text}</div>}
            {repair.reparatur_problem && <p className="text-[11px] text-gray-500 line-clamp-2 leading-relaxed">{repair.reparatur_problem}</p>}
            ...
            <div className="flex items-center justify-between">
              ...
              {saving && <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 rounded-full border-2 border-gray-400 border-t-transparent animate-spin" /><span className="text-[10px] text-gray-400">Speichert…</span></div>}
            </div>
          </div>
        </div>
      </div>
...
function BoardColumn({ status, items, savingId, onStatusChange, newIds }: {
    ...
    <section ref={setNodeRef}
      ...
      <div className="p-2 space-y-2 min-h-[80px]">
        {items.length === 0 ? (
          ...
          items.map((r) => (
            <RepairCard key={r.id} repair={r} saving={savingId === r.id} onStatusChange={onStatusChange} isNew={newIds.has(r.id)} />
          ))
        )}
      </div>
    </section>
...
export default function DashboardClient({ initialItems }: { initialItems: DashboardRepair[] }) {
  ...
  const [newIds,   setNewIds]   = useState<Set<string>>(new Set()); // "NEU" Badge
      ...
      .on(
        ...
        (payload: { eventType: string; new: Record<string, unknown>; old: Record<string, unknown> }) => {
          ...
          if (eventType === "INSERT") {
            ...
            const newRepair: DashboardRepair = {
              id:                 String(newRow.id),
              auftragsnummer:     newRow.auftragsnummer as string | null,
              status:             newRow.status as string | null,
              kunden_name:        newRow.kunden_name as string | null,
              hersteller:         newRow.hersteller as string | null,
              modell:             newRow.modell as string | null,
              geraetetyp:         newRow.geraetetyp as string | null,
              reparatur_problem:  newRow.reparatur_problem as string | null,
              annahme_datum:      newRow.annahme_datum as string | null,
              updated_at:         newRow.updated_at as string | null,
              boardStatus:        normalizeStatus(newRow.status as string | null),
            };
            setItems(prev => [newRepair, ...prev]);
            // NEU Badge für 5 Sekunden
            setNewIds(prev => new Set([...prev, newRepair.id]));
            setTimeout(() => setNewIds(prev => { const n = new Set(prev); n.delete(newRepair.id); return n; }), 5000);
          }
          ...
          if (eventType === "UPDATE") {
            ...
            setItems(prev => prev.map(i =>
              i.id === String(updated.id)
                ? { ...i, ...updated, id: String(updated.id), boardStatus: normalizeStatus(updated.status as string | null) }
                : i
            ));
          }
          ...
          if (eventType === "DELETE") {
            ...
            setItems(prev => prev.filter(i => i.id !== String(deleted.id)));
          }
        }
      )
      .subscribe();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <LiveIndicator />
        <span className="text-[11px] text-gray-400">{items.length} Aufträge gesamt</span>
      </div>
      ...
      <DndContext id="starphone-board" sensors={sensors}
        ...
        <div className="overflow-x-auto pb-4">
          <div className="flex gap-2.5 min-w-max">
            {grouped.map((col) => (
              <BoardColumn key={col.status} status={col.status} items={col.items} savingId={savingId} onStatusChange={updateStatus} newIds={newIds} />
            ))}
          </div>
        </div>
