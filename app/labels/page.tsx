...
import { Tag, Plus, Edit2, Trash2, X, ChevronDown, Printer, CheckSquare, Square, Wifi, WifiOff, AlertCircle, RefreshCw } from 'lucide-react'
...
function TemplateModal({ item, onClose, onSave }: { item: Partial<LabelTemplate> | null; onClose: () => void; onSave: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) { const { error: err } = await supabase.from('label_templates').insert([payload]); if (err) throw err }
      else { const { error: err } = await supabase.from('label_templates').update({ ...payload, updated_at: new Date().toISOString() }).eq('id', item!.id!); if (err) throw err }
      onSave(); onClose()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#13151d] shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4">
            ...
            <div className="flex gap-2">
              {ZUSTAND_OPTIONS.map(z => (
                <button key={z} type="button" onClick={() => set('zustand', z)}
                  className={`flex-1 py-2 rounded-xl border text-xs font-semibold transition ${form.zustand === z ? ZUSTAND_STYLES[z] : 'border-white/10 bg-white/5 text-slate-500 hover:text-white'}`}>
                  {z}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className={lbl}>Verkaufspreis (€) <span className="text-amber-400 normal-case font-normal">wöchentlich anpassen</span></label>
...
function PrinterSettingsModal({ onClose, printerSmall, printerLarge, availablePrinters, onSave }: {
  ...
  const inp = 'w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white outline-none focus:border-violet-500/60 transition appearance-none'

  const Select = ({ value, onChange }: { value: string; onChange: (v: string) => void }) =>
    availablePrinters.length > 0 ? (
      <div className="relative">
        <select value={value} onChange={e => onChange(e.target.value)} className={inp}>
          {availablePrinters.map(p => <option key={p} value={p} className="bg-[#13151d]">{p}</option>)}
        </select>
        <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-500 pointer-events-none" />
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#13151d] shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/8">
          <h2 className="text-base font-semibold text-white">Druckereinstellungen</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/8 transition"><X className="w-4 h-4" /></button>
        </div>
        <div className="px-6 py-5 space-y-4">
          <p className="text-xs text-slate-500">Drucker werden über macOS CUPS erkannt. Namen wie in Systemeinstellungen → Drucker & Scanner.</p>
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wide">Kleines Etikett – SLP-TX223 (50×30mm)</label>
            <Select value={small} onChange={setSmall} />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wide">Großes Etikett – SLP-TX403 (90×60mm)</label>
            <Select value={large} onChange={setLarge} />
          </div>
          {availablePrinters.length === 0 && (
            <div className="px-4 py-3 rounded-xl bg-amber-500/8 border border-amber-500/20 text-xs text-amber-400">
              ⚠️ Keine Drucker erkannt. USB verbinden → Systemeinstellungen → Drucker & Scanner → hinzufügen.
            </div>
          )}
        </div>
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/8">
          <button onClick={onClose} className="px-4 py-2 rounded-xl text-sm text-slate-400 hover:text-white hover:bg-white/8 transition">Abbrechen</button>
          <button onClick={() => { onSave(small, large); onClose() }} className="px-5 py-2 rounded-xl text-sm font-medium bg-gradient-to-r from-violet-600 to-indigo-600 text-white hover:opacity-90 transition">
            Speichern
          </button>
        </div>
      </div>
...
function PrintQueueModal({ jobs, onClose, onPrint }: { jobs: PrintJob[]; onClose: () => void; onPrint: (j: PrintJob[]) => void }) {
  const [queue, setQueue] = useState<PrintJob[]>(jobs)
  const upd = (idx: number, val: number) => setQueue(q => q.map((j, i) => i === idx ? { ...j, anzahl: Math.max(1, val) } : j))
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#13151d] shadow-2xl">
        ...
        <div className="px-6 py-4 space-y-2 max-h-80 overflow-y-auto">
          {queue.length === 0 ? <p className="text-sm text-slate-500 text-center py-8">Keine Etiketten</p>
            : queue.map((job, idx) => (
              <div key={idx} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/4 border border-white/8">
                <div className={`px-2 py-0.5 rounded-md text-xs font-medium border ${job.type === 'small' ? 'bg-violet-500/15 text-violet-400 border-violet-500/25' : 'bg-indigo-500/15 text-indigo-400 border-indigo-500/25'}`}>
                  {job.type === 'small' ? 'Klein' : 'Groß'}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white truncate">{job.template.hersteller} {job.template.modell}</p>
                  <p className="text-xs text-slate-500">{[job.template.speicher, job.template.zustand].filter(Boolean).join(' · ')}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => upd(idx, job.anzahl - 1)} className="w-6 h-6 rounded-lg bg-white/8 text-slate-400 hover:text-white flex items-center justify-center transition">−</button>
                  <span className="w-6 text-center text-sm text-white font-medium">{job.anzahl}</span>
                  <button onClick={() => upd(idx, job.anzahl + 1)} className="w-6 h-6 rounded-lg bg-white/8 text-slate-400 hover:text-white flex items-center justify-center transition">+</button>
                </div>
                <button onClick={() => setQueue(q => q.filter((_, i) => i !== idx))} className="p-1 text-slate-600 hover:text-red-400 transition"><X className="w-3.5 h-3.5" /></button>
              </div>
            ))}
        </div>
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/8">
          ...
          <button onClick={() => onPrint(queue)} disabled={queue.length === 0}
            ...
            <Printer className="w-4 h-4" />Jetzt drucken
          </button>
        </div>
      </div>
    </div>
...
export default function LabelsPage() {
  ...
  const checkPrinters = useCallback(async () => {
    try {
      ...
      if (json.ok && json.printers?.length > 0) { setAvailablePrinters(json.printers); setPrinterOnline(true) }
  ...
  const savePrinterSettings = (s: string, l: string) => {
  ...
  const toggleAll = () => selected.size === templates.length ? setSelected(new Set()) : setSelected(new Set(templates.map(t => t.id)))
  ...
  function buildLargeZPL(t: LabelTemplate): string {
    const preis = t.verkaufspreis != null ? `${t.verkaufspreis.toFixed(2)} EUR` : ''
    const details = [t.speicher ? `Speicher: ${t.speicher}` : null, t.megapixel ? `${t.megapixel} MP` : null,
      t.garantie ? `${t.garantie} Garantie` : null, t.farbe || null, t.zustand || null].filter(Boolean)
    return ['^XA','^CI28','^CF0,48',`^FO20,15^FD${t.hersteller}^FS`,'^CF0,40',`^FO20,70^FD${t.modell}^FS`,
      '^FO20,120^GB680,2,2^FS','^CF0,22', details.map((v, i) => `^FO${20 + i * 150},130^FD${v}^FS`).join('\n'),
  ...
  return (
    <div className="min-h-screen bg-[#0d0f14]">
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-6 xl:px-8 py-8">
        ...
        {printResult && (
          <div className={`mb-4 flex items-center justify-between px-4 py-3 rounded-xl border text-sm ${printResult.success ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
            ...
            <button onClick={() => setPrintResult(null)} className="ml-4 opacity-60 hover:opacity-100"><X className="w-4 h-4" /></button>
          </div>
        )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 flex items-center gap-2">
                ...
                {printerOnline === null ? <span className="text-slate-600">Prüfe Drucker...</span>
                  : printerOnline
                    ? <span className="text-green-500 flex items-center gap-1"><Wifi className="w-3 h-3" />{availablePrinters.length} Drucker bereit</span>
                    : <span className="text-red-400 flex items-center gap-1"><WifiOff className="w-3 h-3" />Kein Drucker</span>}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={checkPrinters} title="Drucker neu prüfen" className="p-2 rounded-xl border border-white/10 bg-white/5 text-slate-400 hover:text-white transition">
              <RefreshCw className="w-4 h-4" />
            </button>
            <button onClick={() => setShowSettings(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-slate-400 hover:text-white text-xs transition">
              <Printer className="w-3.5 h-3.5" />Drucker
            </button>
          ...
          <div className="mb-4 flex flex-wrap items-center gap-3 px-4 py-3 rounded-xl bg-indigo-500/10 border border-indigo-500/20">
            ...
            <div className="flex items-center gap-1 ml-auto">
              <div className="flex rounded-lg bg-white/5 border border-white/10 overflow-hidden text-xs">
                <button onClick={() => setSelectedType('small')} className={`px-3 py-1.5 font-medium transition ${selectedType === 'small' ? 'bg-violet-500/20 text-violet-300' : 'text-slate-400 hover:text-white'}`}>Klein (TX223)</button>
                <button onClick={() => setSelectedType('large')} className={`px-3 py-1.5 font-medium transition ${selectedType === 'large' ? 'bg-indigo-500/20 text-indigo-300' : 'text-slate-400 hover:text-white'}`}>Groß (TX403)</button>
              </div>
        ...
        {printerOnline === false && (
          <div className="mb-4 flex items-start gap-3 px-4 py-3 rounded-xl bg-amber-500/8 border border-amber-500/20 text-sm text-amber-400">
            <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
            <div>
              <p className="font-medium">Kein Drucker erkannt</p>
              <p className="text-xs text-amber-400/70 mt-0.5">Drucker per USB verbinden → Systemeinstellungen → Drucker & Scanner → hinzufügen → <RefreshCw className="w-3 h-3 inline" /> klicken</p>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/8 flex items-center justify-center mb-4"><Tag className="w-7 h-7 text-slate-600" /></div>
                  ...
                  return (
                    <tr key={t.id} onClick={() => toggleSelect(t.id)} className={`group cursor-pointer transition ${isSelected ? 'bg-indigo-500/8' : 'hover:bg-white/3'}`}>
                      ...
                      <td className="px-4 py-3.5">
                        {t.zustand
                          ? <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold border ${ZUSTAND_STYLES[t.zustand] ?? 'bg-white/10 text-slate-400 border-white/10'}`}>{t.zustand}</span>
                          : '—'}
                      </td>
                      <td className="px-4 py-3.5">
      ...
      {showQueue && <PrintQueueModal jobs={templates.filter(t => selected.has(t.id)).map(t => ({ template: t, type: selectedType, anzahl: 1 }))} onClose={() => setShowQueue(false)} onPrint={handlePrint} />}
      {showSettings && <PrinterSettingsModal onClose={() => setShowSettings(false)} printerSmall={printerSmall} printerLarge={printerLarge} availablePrinters={availablePrinters} onSave={savePrinterSettings} />}
    </div>
  )
}