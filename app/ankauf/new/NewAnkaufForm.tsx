...
const ZUSTAENDE = [
  { value: "neu", label: "Neu", cls: "border-blue-400 bg-blue-50 text-blue-700" },
  { value: "gebraucht", label: "Gebraucht", cls: "border-amber-400 bg-amber-50 text-amber-700" },
  { value: "defekt", label: "Defekt", cls: "border-red-400 bg-red-50 text-red-700" },
];
...
export default function NewAnkaufForm() {
  ...
  const [success, setSuccess] = useState<{ id: string; nummer: string; preis: string } | null>(null);
  ...
  async function uploadAusweis(file: File) {
    ...
    if (!error) { const { data } = supabase.storage.from("ankauf-docs").getPublicUrl(path); setAusweisVorne(data.publicUrl); }
    setUploadingAusweis(false);
  }

  async function handleSubmit() {
    if (!kundenName.trim()) { setError("Name eingeben"); return; }
    if (!kundenTelefon.trim()) { setError("Telefon eingeben"); return; }
    ...
    const { data, error: err } = await supabase.from("ankauf").insert({
      ankauf_nummer: ankaufNummer, kunden_name: kundenName.trim(),
      kunden_telefon: kundenTelefon || null, ausweis_nummer: ausweisNummer || null,
      customer_id: selectedCustomer?.id ?? null, ausweis_vorne: ausweisVorne,
      geraetetyp, hersteller: hersteller.trim(), modell: modell.trim(),
      zustand, notiz: notiz || null, ankauf_preis: parseFloat(ankaufPreis), status: "offen",
    }).select("id").single();

    setSaving(false);
    if (err || !data) { setError(err?.message ?? "Fehler"); return; }
    setSuccess({ id: data.id, nummer: ankaufNummer, preis: parseFloat(ankaufPreis).toFixed(2) });
  }
  ...
  function resetForm() {
    ...
    setNotiz(""); setAnkaufPreis(""); setSuccess(null); setError("");
  }
      ...
      <main className="min-h-screen bg-white flex items-center justify-center p-4">
        <div className="w-full max-w-sm text-center space-y-5">
          ...
          <div className="flex gap-2">
            <button onClick={resetForm} className="flex-1 h-10 rounded-xl bg-black text-white text-sm font-medium hover:bg-gray-900 transition-colors">Neuer Ankauf</button>
            <Link href={`/ankauf/${success.id}`} className="flex-1 h-10 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors flex items-center justify-center">Details vervollständigen</Link>
          </div>
        </div>
      </main>
    );
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div className="p-4 space-y-3">
                ...
                {showDropdown && customerResults.length > 0 && (
                  <div className="absolute z-10 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                    {customerResults.map(c => (
                      <button key={c.id} onClick={() => selectCustomer(c)} className="w-full text-left px-3 py-2 hover:bg-gray-50 text-[12px] border-b border-gray-50 last:border-b-0">
                        <span className="font-medium text-gray-900">{c.first_name} {c.last_name}</span>{c.phone && <span className="text-gray-400 ml-2">{c.phone}</span>}
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div><label className={labelCls}>Ausweisnummer</label><input type="text" value={ausweisNummer} onChange={e => setAusweisNummer(e.target.value)} className={inputCls} /></div>
              <div>
                ...
                {ausweisVorne ? (
                  ...
                  <label className="inline-flex h-9 px-4 rounded-lg border border-gray-200 text-[12px] text-gray-600 hover:bg-gray-50 transition-colors items-center gap-2 cursor-pointer">
                    ...
                    <input type="file" accept="image/*" capture="environment" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) uploadAusweis(f); }} />
                  </label>
                )}
              </div>
            </div>
            ...
            <div className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {DEVICE_TYPES.map(t => (
                    <button key={t} type="button" onClick={() => setGeraetetyp(t)}
                      className={`h-11 rounded-xl text-[12px] font-medium border-2 transition-all ${geraetetyp === t ? "bg-black text-white border-black" : "bg-white text-gray-600 border-gray-200 hover:border-gray-400"}`}>{t}</button>
                  ))}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {ZUSTAENDE.map(z => (
                    <button key={z.value} type="button" onClick={() => setZustand(z.value)}
                      className={`h-10 rounded-xl text-[12px] font-medium border-2 transition-all ${zustand === z.value ? z.cls : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>{z.label}</button>
                  ))}
                </div>
              </div>
              <div><label className={labelCls}>Notiz (optional)</label><textarea value={notiz} onChange={e => setNotiz(e.target.value)} rows={2} placeholder="Kratzer, Mängel..." className="w-full px-3 py-2 text-[12px] rounded-lg border border-gray-200 bg-white text-gray-900 placeholder-gray-300 focus:outline-none focus:ring-1 focus:ring-gray-300 resize-none" /></div>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div className="p-4">
              <div className="relative max-w-xs">
                <input type="number" step="0.01" min="0" value={ankaufPreis} onChange={e => setAnkaufPreis(e.target.value)} placeholder="0,00"
