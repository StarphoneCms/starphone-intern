...
export default function AnkaufClient() {
  ...
  const filtered = filter === "Alle" ? items : items.filter(a => a.status === TAB_MAP[filter]);
        ...
        <div className="flex gap-1 mb-4 overflow-x-auto">
          {TABS.map(t => (
            <button key={t} onClick={() => setFilter(t)}
              ...
              {t} {t !== "Alle" && <span className="text-[10px] ml-1 opacity-60">({items.filter(a => a.status === TAB_MAP[t]).length})</span>}
            </button>
          ))}
        </div>
                ...
                {filtered.map(a => {
                  ...
                  return (
                    <tr key={a.id} onClick={() => router.push(`/ankauf/${a.id}`)}
                      className={`hover:bg-gray-50 transition-colors cursor-pointer ${a.status === "offen" ? "bg-amber-50/30" : ""}`}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium border ${ZUSTAND_CLS[a.zustand] ?? "text-gray-600 bg-gray-50 border-gray-200"}`}>{a.zustand}</span>
                      </td>
                      <td className="px-4 py-3 text-right font-semibold text-gray-900">{Number(a.ankauf_preis).toFixed(2)} €</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium border ${sc.cls}`}>{sc.label}</span>
                      </td>
